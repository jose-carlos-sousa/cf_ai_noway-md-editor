...
import { exportMarkdown, extractMarkdown, normalizeMarkdown, diffLinesLCS } from "../utils/markdownUtils";
...
// Separate endpoints:
// - `LLM_WORKER_URL` is used for AI/LLM calls
// - `KV_WORKER_URL` is used for saving/loading markdown to KV
const LLM_WORKER_URL = process.env.NEXT_PUBLIC_LLM_URL || "https://my-llama-xd.2409jmsousa.workers.dev";
const KV_WORKER_URL = process.env.NEXT_PUBLIC_WORKER_URL || "https://noway-md-kv-api.2409jmsousa.workers.dev";
...
export default function Home() {
	...
	// Extracts explanation from the LLM message ( stuff that is not the markdown itself)
	...
	const handleSave = async () => {
		if (!username) {
			alert("Please enter a username to save your data.");
			return;
		}
			try {
				console.log('Saving markdown for user:', username);
				const response = await fetch(`${KV_WORKER_URL}/api/markdown`, {
					method: "POST",
					headers: { "Content-Type": "application/json" },
	...
	// Fixed useEffect - runs when username is submitted
	useEffect(() => {
		const fetchMarkdown = async () => {
			// Only fetch when username is submitted
			if (!username || !isUsernameSubmitted) return;
			
			console.log('Fetching markdown for:', username);
			
				try {
				const res = await fetch(`${KV_WORKER_URL}/api/markdown?username=${encodeURIComponent(username)}`);
				...
				if (res.ok) {
					...
					if (text) {
						setMarkdown(text);
						console.log('Loaded saved markdown');
					} else {
						console.log('No saved markdown found, using default');
					}
				}
			} catch (err) {
		...
		fetchMarkdown();
	}, [username, isUsernameSubmitted]); // Added isUsernameSubmitted dependency
			...
			{!isUsernameSubmitted ? (
				<div className="flex items-center justify-center h-screen">
					<form
						...
						className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg w-full max-w-md"
					>
						<h2 className="text-2xl font-bold mb-4 text-slate-900 dark:text-white">Enter Username</h2>
						<input
							type="text"
							value={username}
							onChange={(e) => setUsername(e.target.value)}
							placeholder="Enter your username"
							className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-900"
						/>
						<button
							type="submit"
